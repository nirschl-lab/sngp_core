...
class LitModuleBase(LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.predict_acc = Accuracy(task="multiclass", num_classes=self.num_classes)
        self.predict_precision = MulticlassPrecision(num_classes=self.num_classes, average='macro')
        self.predict_recall = MulticlassRecall(num_classes=self.num_classes, average='macro')
        self.predict_f1 = MulticlassF1Score(num_classes=self.num_classes, average='macro')
    ...
    def predict_step(self, batch, batch_idx):
        ...
        self.predict_acc(preds, targets)
        self.predict_precision(preds, targets)
        self.predict_recall(preds, targets)
        self.predict_f1(preds, targets)
    ...
    def on_predict_end(self):
        ...
        acc = self.predict_acc.compute()  # Get current validation accuracy
        precision = self.predict_precision.compute()
        recall = self.predict_recall.compute()
        f1 = self.predict_f1.compute()
        
        probs_all = torch.cat(self._predict_probs).numpy()
        prediction_prob_score = np.max(probs_all, axis=1)
        prediction = np.argmax(probs_all, axis=-1)
        prediction = [self.idx_to_classes[idx] for idx in prediction]

        targets = torch.cat(self._predict_targets).numpy()
        targets = [self.idx_to_classes[idx] for idx in targets]
        ...
        data_dict = {
            ...
            'prob_score': prediction_prob_score
        }
        self._log_csv_artifact(data_dict)
        ...
        if self.logger and hasattr(self.logger, 'experiment'):
            ...
            "predict/acc": acc.item(),
            "predict/precision": precision.item(),
            "predict/recall": recall.item(),
            "predict/f1": f1.item()
        })
    ...
    def load_state_dict(self, state_dict, strict=False):
        ...
        for key, value in state_dict.items():
            ...
            if (key == "criterion.nll_loss.weight" or key == "criterion.weight") and self.class_weights is None:
