...
class RandomFourierFeatures(nn.Module):
    ...
    def __init__(
        ...
        if out_features <= 0:
            raise ValueError(
                "`out_features` should be a positive integer. Given: {}.".format(
                    out_features
                )
            )
            ...
            if kernel_type.lower() not in _SUPPORTED_RBF_KERNEL_TYPES:
                raise ValueError(
                    "Unsupported kernel type: '{}'. Supported kernel types: {}.".format(
                        kernel_type, _SUPPORTED_RBF_KERNEL_TYPES
                    )
                )

        if kernel_scale is not None and kernel_scale <= 0.0:
            raise ValueError(
                "When provided, `scale` should be a strictly positive float. Given: {}.".format(
                    kernel_scale
                )
            )
        ...
        if kernel_type.lower() == "gaussian":
            ...
            self.register_buffer(
                "weight",
                torch.tan(math.pi * (torch.rand(in_features, out_features) - 0.5)),
            )
        else:
        ...
        if kernel_scale is None:
            kernel_scale = (
                math.sqrt(in_features / 2.0) if kernel_type == "gaussian" else 1.0
            )
