#!/bin/bash
# Script to run evaluation with specific parameters
set -e

# Parameters
LOGGER="wandb"

DATA="wbc_classifier"

# wong baseline model path
CKPT_PATH="nirschl-lab/wbc_challenge/model-nutvnaqy:v1"

MODEL="wbc_FL_CE"
# MODEL="swin_baseline_classifier"

WANDB_PROJECT="wbc_challenge"
WANDB_GROUP="test_resnet18"
EXPERIMENT_NAME="resnet18_FL_CE"
ACCELERATOR="gpu"
BATCH_SIZE=2048
N_CLASSES=13
model_name="resnet18"
# model_name="swin_t"
csv_save_path="csv/wbc/resnet18_FL_CE/"
data_dir='/data1/shared/data/wbc-bench-2026/'

# Define tags as a variable
TAGS="[\"baseline\", \"resnet18\", \"CE\"]"

uv run src/eval.py \
    logger=${LOGGER} \
    ckpt_path=${CKPT_PATH} \
    data=${DATA} \
    data.datamodule.test_batch_size=${BATCH_SIZE} \
    data.datamodule.num_classes=${N_CLASSES} \
    model=${MODEL} \
    model.CE_class_weights=null \
    model.FL_class_weights=null \
    model.num_classes=${N_CLASSES} \
    model.net.arch=${model_name} \
    model.net.num_classes=$N_CLASSES \
    model.net.pretrained=true \
    model.log_csv=True \
    model.csv_save_path="${csv_save_path}" \
    trainer.accelerator="${ACCELERATOR}" \
    logger.wandb.group="${WANDB_GROUP}" \
    +logger.wandb.name="${EXPERIMENT_NAME}" \
    logger.wandb.tags="${TAGS}" \
	++logger.wandb.project="${WANDB_PROJECT}"